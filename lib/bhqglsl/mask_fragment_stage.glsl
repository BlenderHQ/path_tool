...
/**
 * Приблизна оцінка (3 точністю IEEE-754 float) глибини фрагмента відносно
 * текстури глибини.
 **/
...
/**
 * Обертання двовимірного вектора.
 **/
...
void checker_pattern(out float pattern, in float scale) {
  pattern = clamp(
      mod(floor(scale * gl_FragCoord.x) + floor(scale * gl_FragCoord.y), /* Послідовність шашок = */ 2.0), 0.0, 1.0);
}
